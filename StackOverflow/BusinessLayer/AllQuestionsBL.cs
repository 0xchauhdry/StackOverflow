...
{
    public class AllQuestionsBL
    {
        public List<Question> GetAllQuestions()
        {
            List<Question> Questions = new List<Question>();
            AllQuestionsDAL aqDAL = new AllQuestionsDAL();
            DataTable data = aqDAL.GetQuestions();
            ...
            for (int i = 0; i < data.Rows.Count; i++)
            {
                ...
                DataTable votedata = aqDAL.VoteCount(id);
                ...
                DataTable ansdata = aqDAL.AnswerCount(id);
                ...
                DataTable userdata = aqDAL.GetUserbyID(userid);
                ...
                DataTable tagdata = aqDAL.GetQuestionTags(id);
                if (tagdata.Rows.Count > 0)
                {
                    for (int j = 0; j < tagdata.Rows.Count; j++)
                    {
                        if (!(tagdata.Rows[j]["TagID"] is DBNull))
                        {
                            ...
                            DataTable tdata = aqDAL.GetTagsbyID(tagid);
