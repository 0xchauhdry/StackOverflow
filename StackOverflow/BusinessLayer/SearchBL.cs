...
namespace StackOverflow.BusinessLayer
{
    public class SearchBL
    {
        public List<Question> GetQuestions(string search)
        {
            ...
            DataTable data = sDAL.GetQuestions(search);
            ...
            for (int i = 0; i < data.Rows.Count; i++)
            {
                ...
                DataTable votedata = sDAL.VoteCount(id);
                ...
                DataTable ansdata = sDAL.AnswerCount(id);
                ...
                DataTable userdata = sDAL.GetUserbyID(userid);
                ...
                DataTable tagdata = sDAL.GetQuestionTags(id);
                if (tagdata.Rows.Count > 0)
                {
                    for (int j = 0; j < tagdata.Rows.Count; j++)
                    {
                        if (!(tagdata.Rows[j]["TagID"] is DBNull))
                        {
                            ...
                            DataTable tdata = sDAL.GetTagsbyID(tagid);
